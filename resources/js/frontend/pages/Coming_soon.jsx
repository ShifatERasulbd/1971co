...
const launchDate = new Date('2026-09-25T00:00:00-04:00').getTime(); 
       ...
       const handleEmailSubmit = (e) => {
                ...
                setErrorMessage('Access denied. Please enter a valid authorized email.');
                }
            };
        ...
        return (
            ...
            <div className="mx-auto flex w-full max-w-4xl flex-col items-center justify-center px-6 py-16 text-center text-white">
                ...
                {/* Countdown Timer */}
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 sm:gap-6 mb-10 w-full max-w-2xl">
                ...
                {/* Email Access Form */}
                <form onSubmit={handleEmailSubmit} className="flex flex-col sm:flex-row items-center gap-3 w-full max-w-md mb-6">
                <input
                    type="email"
                    value={emailInput}
                    ...
                    required
                    className="w-full rounded-xl bg-white/10 border border-white/20 px-4 py-3 text-white placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-white/50 backdrop-blur-sm"
                />
                <button
                    type="submit"
                    className="w-full sm:w-auto rounded-xl bg-white px-6 py-3 font-semibold text-zinc-900 transition hover:bg-zinc-200 shadow-md"
                >
                    Enter
                </button>
                </form>
                ...
                {errorMessage && (
                <p className="text-sm font-medium text-red-400 mb-4">{errorMessage}</p>
                )}

                <div className="text-sm sm:text-base text-zinc-300 font-medium">
                Official Launch Date: <span className="text-white font-semibold">25 September 2026</span>
                </div>
            </div>
            </section>
        );
